...
module.exports = async function runGame(socket, io, room) {
  ...
  room.players.forEach((player) => {
    ...
    // sockets[player.id].emit("choose-words", words);
    // ...
    // await getDataFromRoom(room, sockets, "word", 0); 
    // (don't use `phasesElapsed` in case I want to toggle that in the future for odd player numbers)
  ...
  async function initiateGuessPhase() {
    ...
    room.players.forEach((player, idx) => {
      ...
      const dataURL = rd[rd.length - 1].data;

      sockets[player.id].emit("load-guessing-phase", dataURL);
    });
  }
  ...
  async function initiateDrawingPhase() {
    ...
    room.players.forEach((player, idx) => {
      ...
      // } else {
        ...
        const index = wrapIndex(idx - phasesElapsed, room.players.length);
        const rd = room.players[index].replayData;
        const word = rd[rd.length - 1].data;
      // }
