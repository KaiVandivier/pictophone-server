...
function clearReadies(room) {
  ...
  // I'm actually not sure how this is using `io` here; I thought it would be out of scope
...
module.exports = function runGame(socket, io, room) {
  ...
  function initiateGuessPhase() {
    room.players.forEach((player, idx) => {
      ...
      // send image from previous player's drawing
    ...
    room.players.forEach((player, idx) => {
      sockets[player.id].once("guess-data", (guess) => {
        ...
        playerToSendDataTo.replayData.rounds.push({
          type: "guess",
          data: guess,
          playerName: player.name || player.id, // This needs to be previous/next player's id
        });
