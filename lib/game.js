...
module.exports = async function runGame(socket, io, room) {
  ...
  // const dataFetchTest = await new Promise((resolve, reject) => {
  //   socket.emit("get-data", resolve)
  // })
  // console.log(dataFetchTest);

  const clientSockets = {};
  await io.in(room.id).clients((err, clients) => {
    if (err) throw err;
    console.log("Clients: ", clients); // ["id1", "id2"]
    clients.forEach((id) => (clientSockets[id] = io.in(room.id).connected[id]));
  });
  ...
  const ioSockets = io.of("/").connected;
  console.log("Sockets: ", sockets);
  console.log("ioSockets: ", ioSockets);
  ...
  async function initiateGuessPhase() {
    await countdown(5, "Time to guess!");
    room.players.forEach((player, idx) => {
